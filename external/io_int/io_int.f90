...
33    CONTINUE
      IF ( retval < 0 )  THEN
        CALL wrf_error_fatal3("<stdin>",78,&
"io_int.F90: int_get_fresh_handle() can not get new handle")
      ENDIF
...
"io_int.F90: ext_int_get_next_time: invalid data handle" )
  ENDIF
  IF ( .NOT. int_handle_in_use( DataHandle ) ) THEN
    CALL wrf_error_fatal3("<stdin>",541,&
"io_int.F90: ext_int_get_next_time: DataHandle not opened" )
  ENDIF
...
SUBROUTINE ext_int_set_time ( DataHandle, DateStr, Status )
  ...
  CALL int_gen_ti_header_char( hdrbuf, hdrbufsize, itypesize,        &
                               DataHandle, "TIMESTAMP", "", TRIM(DateStr), 110 )
...
"io_int.F90: ext_int_get_var_info: invalid data handle" )
  ENDIF
  IF ( .NOT. int_handle_in_use( DataHandle ) ) THEN
    CALL wrf_error_fatal3("<stdin>",645,&
"io_int.F90: ext_int_get_var_info: DataHandle not opened" )
  ENDIF
  inttypesize = itypesize
  realtypesize = rtypesize
  DO WHILE ( .TRUE. )
...
"io_int.F90: ext_int_get_next_var: invalid data handle" )
  ENDIF
  IF ( .NOT. int_handle_in_use( DataHandle ) ) THEN
    CALL wrf_error_fatal3("<stdin>",732,&
"io_int.F90: ext_int_get_next_var: DataHandle not opened" )
  ENDIF
  inttypesize = itypesize
  realtypesize = rtypesize
  DO WHILE ( .TRUE. )
...
'io_int.F90: ext_int_get_dom_ti_real: loccount .GT. Count' )
              ENDIF
              keepgoing = .false. ;  Status = 0
            ENDIF
          ELSE IF ( .NOT. ( code .EQ. 180 .OR. code .EQ. 200 .OR. &
                            code .EQ. 220    .OR. code .EQ. 160  .OR. &
                            code .EQ. 280 .OR. code .EQ. 300 .OR. &
                            code .EQ. 320    .OR. code .EQ. 260  .OR. &
                            code .EQ. 240                                  ) ) THEN
            BACKSPACE ( unit=DataHandle )
    ...
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( int_ok_to_put_dom_ti( DataHandle ) ) THEN
        CALL int_gen_ti_header( hdrbuf, hdrbufsize, itypesize, rtypesize, &
                                DataHandle, Element, Data, Count, 140 )
        WRITE( unit=DataHandle ) hdrbuf
      ENDIF
    ENDIF
  ENDIF
...
'ext_int_get_dom_ti_double not supported yet')
  ENDIF
RETURN
...
'ext_int_put_dom_ti_double not supported yet')
  ENDIF
RETURN
...
'io_int.F90: ext_int_get_dom_ti_integer: loccount .GT. Count' )
              ENDIF
              keepgoing = .false. ;  Status = 0
            ENDIF

          ELSE IF ( .NOT. ( code .EQ. 140    .OR.   code .EQ. 200 .OR. &
                            code .EQ. 220    .OR.   code .EQ. 160  .OR. &
                            code .EQ. 240    .OR.   code .EQ. 300 .OR. &
                            code .EQ. 320    .OR.   code .EQ. 260  .OR. &
                            code .EQ. 280 )                                           ) THEN
            BACKSPACE ( unit=DataHandle )
            keepgoing = .false. ; Status = 1
    ...
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( int_ok_to_put_dom_ti( DataHandle ) ) THEN
        CALL int_gen_ti_header( hdrbuf, hdrbufsize, itypesize, itypesize, &
                                DataHandle, Element, Data, Count, 180 )
        WRITE( unit=DataHandle ) hdrbuf 
      ENDIF
    ENDIF
  ENDIF
      ...
      DO WHILE ( keepgoing )
        ...
        IF ( istat .EQ. 0 ) THEN
          ...
          IF ( code .EQ. 220 ) THEN
            ...
            ENDIF
          ELSE IF ( .NOT. ( code .EQ. 140    .OR. code .EQ. 200 .OR. &
                            code .EQ. 180 .OR. code .EQ. 160  .OR. &
                            code .EQ. 240    .OR. code .EQ. 300 .OR. &
                            code .EQ. 280 .OR. code .EQ. 260  .OR. &
                            code .EQ. 320                                             ) ) THEN
            BACKSPACE ( unit=DataHandle )
            keepgoing = .false. ; Status = 1
          ENDIF
...
'ext_int_get_dom_td_double not supported yet')
RETURN
  ...
  IF ( int_valid_handle ( Datahandle ) ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      CALL int_gen_td_header_char( hdrbuf, hdrbufsize, itypesize,  &
                                   DataHandle, DateStr, Element, Data, 320 )
      WRITE( unit=DataHandle ) hdrbuf
    ENDIF
  ENDIF
  Status = 0
...
'ext_int_get_var_ti_double not supported yet')
RETURN
...
'ext_int_put_var_ti_double not supported yet')
RETURN
...
SUBROUTINE ext_int_put_var_ti_char ( DataHandle,Element,  Varname, Data,  Status )
  ...
  IF ( int_valid_handle (DataHandle) ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      CALL int_gen_ti_header_char( hdrbuf, hdrbufsize, itypesize,  &
                              DataHandle, TRIM(Element), TRIM(VarName), TRIM(Data), 420 )
      WRITE( unit=DataHandle ) hdrbuf
    ENDIF
  ENDIF
  Status = 0
...
'ext_int_get_var_td_double not supported yet')
RETURN
...
'ext_int_put_var_td_double not supported yet')
RETURN
...
"io_int.F90: ext_int_read_field: invalid data handle" )
  ENDIF
  IF ( .NOT. int_handle_in_use( DataHandle ) ) THEN
    CALL wrf_error_fatal3("<stdin>",1669,&
"io_int.F90: ext_int_read_field: DataHandle not opened" )
  ENDIF
...
"io_int.F90: ext_int_write_field: invalid data handle" )
  ENDIF
  IF ( .NOT. int_handle_in_use( DataHandle ) ) THEN
    CALL wrf_error_fatal3("<stdin>",1771,&
"io_int.F90: ext_int_write_field: DataHandle not opened" )
  ENDIF
...
'io_int.F90: ext_int_write_field, WRF_DOUBLE not yet supported')
  ELSE IF ( FieldType .EQ. WRF_INTEGER ) THEN
    typesize = itypesize
  ELSE IF ( FieldType .EQ. WRF_LOGICAL ) THEN
    CALL wrf_error_fatal3("<stdin>",1785,&
'io_int.F90: ext_int_write_field, WRF_LOGICAL not yet supported')
  ENDIF
